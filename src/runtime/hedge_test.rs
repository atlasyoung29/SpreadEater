//! Deterministic Layer 1 hedge harness.
//!
//! Loads a post-attribution fill-work-item fixture, wires the real `FillHandler`
//! against a mutable in-process mock exchange, and asserts the downstream hedge
//! path from emitted events plus final risk state.
...
use crate::models::{
    CanonicalMarket, MarketStatus, OrderBookSnapshot, OrderType, PriceLevel, QuoteLeg,
    RewardConfig, Side,
};
use crate::monitor::ErrorLogger;
use crate::runtime::live_engine::{FillHandler, FillWorkItem};
use crate::trading::ctf_merge::PairMerger;
use crate::trading::hedge_executor::HedgeExecutor;
use crate::trading::order_manager::{OrderManager, TrackedOrder};
use crate::trading::positions::PositionManager;
...
pub(crate) fn serialize_optional_decimal<S>(
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioTrackedOrder {
    pub order_id: String,
    #[serde(default)]
...
pub struct ScenarioExchange {
    ...
    #[serde(default)]
    pub trade_lookup: Vec<ScenarioTradeLookupScript>,
    #[serde(default)]
    pub merge: Option<ScenarioMergeBehavior>,
    #[serde(default)]
    pub user_stream_connect_ack_delay_ms: u64,
    #[serde(default)]
    pub actions: Vec<ScenarioExchangeAction>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioMergeBehavior {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioExchangeBooks {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioPriceLevel(
    #[serde(with = "rust_decimal::serde::str")] pub Decimal,
    #[serde(with = "rust_decimal::serde::str")] pub Decimal,
);

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioBalanceStep {
    #[serde(with = "rust_decimal::serde::str")]
    pub amount: Decimal,
    #[serde(default)]
    pub delay_ms: u64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioPositionStep {
    #[serde(with = "rust_decimal::serde::str")]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioOpenOrdersStep {
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioOrderLookupScript {
    pub order_id: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioOrderLookupStep {
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioTradeLookupScript {
    pub trade_id: String,
    #[serde(default)]
    pub responses: Vec<ScenarioTradeLookupStep>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioTradeLookupStep {
    #[serde(default)]
    pub trade: Option<ScenarioTradeRecord>,
    #[serde(default)]
    pub delay_ms: u64,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioTradeRecord {
    pub id: String,
    pub leg: QuoteLeg,
    pub side: Side,
    #[serde(with = "rust_decimal::serde::str")]
    ...
    pub maker_order_id: Option<String>,
    #[serde(default)]
    pub match_time_unix: Option<i64>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioLiveOrder {
    pub id: String,
    pub leg: QuoteLeg,
    #[serde(with = "rust_decimal::serde::str")]
    pub price: Decimal,
...
pub enum ScenarioExchangeAction {
    Place {
        #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioPlacedOrderResponse {
    pub order_id: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioCancelActionResponse {
...
pub struct ScenarioExchangeMutations {
    #[serde(default)]
    ...
    #[serde(default)]
    pub replace_trade_lookup: Vec<ScenarioTradeLookupScript>,
    #[serde(default)]
    pub append_user_stream_messages: Vec<ScenarioUserStreamMessage>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScenarioUserStreamMessage {
    pub text: String,
    #[serde(default)]
    pub delay_ms: u64,
}
...
#[derive(Debug, Clone)]
pub struct ObservedHedgeOutcome {
    pub result_status: Option<String>,
    pub hedge_side: Option<Side>,
    pub planned_hedge_shares: Option<Decimal>,
    pub planned_sellback_shares: Option<Decimal>,
...
struct ScenarioPairMerger {
    behavior: ScenarioMergeBehavior,
}
...
impl HedgeTestHarness {
    pub async fn from_scenario(scenario: HedgeScenario) -> Result<Self> {
        validate_scenario(&scenario)?;

        let mock_server = MockExchangeServer::spawn(&scenario.market, &scenario.exchange).await?;
        info!(mock_url = %mock_server.base_url(), "Mock exchange API server started");
        ...
        let event_producer: Arc<dyn EventProducer> = event_collector.clone();
        ...
        let fill_handler = FillHandler {
            ...
            ctf_merger,
            post_hedge_mode: crate::runtime::live_engine::FillHandlerPostHedgeMode::Normal,
            hedge_locks: Arc::new(RwLock::new(HashMap::new())),
            error_logger,
        };

        let work_item = build_work_item(&scenario);
    ...
    pub async fn run(self) -> Result<HedgeTestResult> {
        let scenario_name = self.scenario.name.clone();
        let expected = self.scenario.expected.clone();

        info!(scenario = %scenario_name, "Running hedge test scenario");

        self.fill_handler
            .handle_fill(self.work_item)
            .await
            .with_context(|| {
                format!("Hedge harness runtime error in scenario {}", scenario_name)
            })?;

        let events = self.event_collector.events();
        let request_log = self.mock_server.request_log().await;
        ...
        let mismatches = compare_expected_to_observed(&expected, &observed, actual_success);
        ...
        if !passed {
            warn!(
                ...
                "Hedge test scenario FAILED"
            );
        } else {
            info!(scenario = %scenario_name, "Hedge test scenario PASSED");
        }
...
pub async fn run_hedge_test(scenario_path: &str) -> Result<HedgeTestResult> {
    let scenario = load_scenario(scenario_path)?;
    let harness = HedgeTestHarness::from_scenario(scenario).await?;
    harness.run().await
}

pub fn print_report(result: &HedgeTestResult) {
    println!("=== Hedge Test: {} ===", result.scenario_name);
    ...
    println!(
        "Risk state: {}",
        if result.halted { "halted" } else { "normal" }
    );
    println!(
        "Observed: result_status={:?}, hedge_side={:?}, planned_hedge_shares={:?}, planned_sellback_shares={:?}, hedge_leg_status={:?}, sellback_leg_status={:?}, post_sync_yes_size={:?}, post_sync_no_size={:?}, post_sync_net_exposure={:?}",
        ...
        result.observed.hedge_leg_status,
        result.observed.sellback_leg_status,
        result.observed.post_sync_yes_size,
        result.observed.post_sync_no_size,
...
pub fn load_scenario(path: &str) -> Result<HedgeScenario> {
    let contents = std::fs::read_to_string(path)
        .with_context(|| format!("Failed to read scenario file: {path}"))?;
    let scenario: HedgeScenario = serde_json::from_str(&contents)
...
#[derive(Default)]
pub(crate) struct InMemoryEventCollector {
    events: Mutex<Vec<EventEnvelope>>,
}

impl InMemoryEventCollector {
    pub(crate) fn events(&self) -> Vec<EventEnvelope> {
        self.events
            .lock()
...
impl MockExchangeServer {
    pub async fn spawn(market: &ScenarioMarket, exchange: &ScenarioExchange) -> Result<Self> {
        ...
        let ws_listener = TcpListener::bind("127.0.0.1:0")
            .await
            .context("Failed to bind mock user stream server")?;
        let ws_addr = ws_listener
            .local_addr()
            .context("Failed to read mock user stream server addr")?;
        let (user_ws_tx, _) = broadcast::channel(64);
        let state = Arc::new(AsyncMutex::new(MockExchangeState::new(
            market,
            exchange,
            user_ws_tx.clone(),
        )));
        ...
        let ws_task = tokio::spawn(async move {
            loop {
                let Ok((socket, _)) = ws_listener.accept().await else {
                    break;
                };
                let mut user_ws_rx = user_ws_tx.subscribe();
                let ack_delay_ms = {
                    let state = state_for_ws.lock().await;
                    state.user_stream_connect_ack_delay_ms
                };
                tokio::spawn(async move {
                    ...
                    let mut ack_timer = Box::pin(tokio::time::sleep(
                        tokio::time::Duration::from_millis(ack_delay_ms),
                    ));

                    loop {
                        tokio::select! {
                            _ = &mut ack_timer, if !ack_sent => {
                                if write
    ...
    pub fn user_ws_url(&self) -> &str {
        &self.user_ws_url
    }

    pub async fn request_log(&self) -> Vec<MockRequestRecord> {
        self.state.lock().await.request_log.clone()
    }

    pub async fn replace_positions(&self, positions: Vec<ScenarioPositionStep>) {
        let mut state = self.state.lock().await;
        state.positions = queue_or_default(
            positions,
    ...
    pub async fn replace_balances(&self, balances: Vec<ScenarioBalanceStep>) {
        let mut state = self.state.lock().await;
        state.balances = queue_or_default(
            balances,
            ScenarioBalanceStep {
                amount: Decimal::ZERO,
                delay_ms: 0,
    ...
    pub async fn emit_user_stream_messages(&self, messages: Vec<ScenarioUserStreamMessage>) {
        let sender = { self.state.lock().await.user_ws_tx.clone() };
        schedule_user_stream_messages(sender, messages);
    }
}
...
struct MockExchangeState {
    ...
    trade_lookup: HashMap<String, VecDeque<ScenarioTradeLookupStep>>,
    actions: VecDeque<ScenarioExchangeAction>,
    request_log: Vec<MockRequestRecord>,
    user_ws_tx: broadcast::Sender<String>,
    user_stream_connect_ack_delay_ms: u64,
}
...
impl MockExchangeState {
    fn new(
        ...
        Self {
            ...
            order_lookup: exchange
                ...
                .collect(),
            trade_lookup: exchange
                .trade_lookup
                .iter()
                .map(|script| {
                    (
                        script.trade_id.clone(),
                        queue_or_default(
                            script.responses.clone(),
                            ScenarioTradeLookupStep {
                                trade: None,
                                delay_ms: 0,
                            },
                        ),
            ...
            request_log: Vec::new(),
            user_ws_tx,
            user_stream_connect_ack_delay_ms: exchange.user_stream_connect_ack_delay_ms,
        }
    }
    ...
    fn apply_mutations(
        ...
        for script in &mutations.replace_trade_lookup {
            self.trade_lookup.insert(
                script.trade_id.clone(),
                queue_or_default(
                    script.responses.clone(),
                    ScenarioTradeLookupStep {
        ...
        mutations.append_user_stream_messages.clone()
    }
}
...
async fn handle_mock_request(
    ...
    let response = if request.method == "GET" && request.path.starts_with("/sampling-markets") {
        (
            "200 OK".to_string(),
            scenario_market_to_sampling_markets_json(&state.market),
            0,
        )
    } else if request.method == "GET" && request.path.starts_with("/fee-rate") {
            ...
            || request.path.starts_with("/neg-risk/balance-allowance"))
    {
    ...
    } else if request.method == "GET" && request.path.starts_with("/data/trades") {
        let trade_id = query_param(&request.path, "id").unwrap_or_default();
        let Some(queue) = state.trade_lookup.get_mut(&trade_id) else {
            return ("200 OK".to_string(), json!([]).to_string());
        };
        let step = next_scripted_step(
            queue,
            ScenarioTradeLookupStep {
        ...
        let body = step
            .trade
            .as_ref()
            .map(|trade| json!([trade_record_to_json(&state.market, trade)]))
            .unwrap_or_else(|| json!([]));
        ("200 OK".to_string(), body.to_string(), step.delay_ms)
...
fn schedule_user_stream_messages(
    sender: broadcast::Sender<String>,
    messages: Vec<ScenarioUserStreamMessage>,
) {
    for message in messages {
        let sender = sender.clone();
        tokio::spawn(async move {
            if message.delay_ms > 0 {
                sleep(Duration::from_millis(message.delay_ms)).await;
            }
...
fn consume_place_action(
    ...
    let user_ws_messages = state.apply_mutations(&mutations);
    schedule_user_stream_messages(state.user_ws_tx.clone(), user_ws_messages);
...
pub(crate) fn build_test_credentials() -> ApiCredentials {
    ApiCredentials {
...
pub(crate) fn validate_scenario_market(market: &ScenarioMarket) -> Result<()> {
...
fn validate_scenario(scenario: &HedgeScenario) -> Result<()> {
    validate_scenario_market(&scenario.market)?;
    if scenario.trigger.work_item.match_source.trim().is_empty() {
        bail!("Scenario trigger.work_item.match_source must not be empty");
    }
    Ok(())
}

pub(crate) fn build_canonical_market(market: &ScenarioMarket) -> CanonicalMarket {
    CanonicalMarket {
...
pub(crate) fn scenario_book_to_snapshot(token_id: &str, book: &ScenarioBook) -> OrderBookSnapshot {
    OrderBookSnapshot {
...
fn scenario_market_to_sampling_markets_json(market: &ScenarioMarket) -> String {
    json!({
        "data": [{
            "condition_id": market.condition_id.clone(),
            "market_slug": format!("{}-slug", market.condition_id),
            ...
            "neg_risk": false,
            "minimum_tick_size": market.tick_size.parse::<f64>().unwrap_or(0.01),
            "tokens": [
                {
            ...
            "rewards": {
                "rates": [{
                    "rewards_daily_rate": market.daily_reward_total.to_string().parse::<f64>().unwrap_or(0.0)
                }],
                "min_size": 0.0,
                "max_spread": market.max_spread.to_string().parse::<f64>().unwrap_or(0.0) * 100.0
            },
            "end_date_iso": null
        }]
    })
...
fn build_work_item(scenario: &HedgeScenario) -> FillWorkItem {
    ...
    let tracked = TrackedOrder {
        ...
        trace_id: tracked_order
            .trace_id
            .clone()
            .unwrap_or_else(|| format!("hedge-test-trace-{}", Uuid::new_v4())),
        condition_id: scenario.market.condition_id.clone(),
        created_at,
        leg: tracked_order.leg,
        token_id: token_id_for_leg(tracked_order.leg, &scenario.market),
        opposite_token_id: opposite_token_id_for_leg(tracked_order.leg, &scenario.market),
    ...
    let trade = TradeEvent {
        ...
        condition_id: trade
            .condition_id
            .clone()
            .unwrap_or_else(|| scenario.market.condition_id.clone()),
        asset_id: trade
            .asset_id
            .clone()
            .unwrap_or_else(|| token_id_for_leg(tracked_order.leg, &scenario.market)),
        side: trade.side,
        price: trade.price,
        size: trade.size,
        outcome: trade
            .outcome
            .clone()
            .unwrap_or_else(|| outcome_for_leg(tracked_order.leg).to_string()),
...
pub(crate) fn side_for_leg(leg: QuoteLeg) -> Side {
    match leg {
...
pub(crate) fn outcome_for_leg(leg: QuoteLeg) -> &'static str {
    match leg {
...
pub(crate) fn token_id_for_leg(leg: QuoteLeg, market: &ScenarioMarket) -> String {
    match leg {
...
pub(crate) fn opposite_token_id_for_leg(leg: QuoteLeg, market: &ScenarioMarket) -> String {
    match leg {
...
fn trade_record_to_json(market: &ScenarioMarket, trade: &ScenarioTradeRecord) -> Value {
    let asset_id = token_id_for_leg(trade.leg, market);
    let outcome = outcome_for_leg(trade.leg);
    let maker_orders = trade
        ...
        .map(|order_id| {
            vec![json!({
                "order_id": order_id,
                "owner": "mock-maker-owner",
                "maker_address": TEST_ADDRESS,
                "matched_amount": trade.size.to_string(),
    ...
    json!({
        ...
        "price": trade.price.to_string(),
        "status": trade.status,
        "match_time": chrono::DateTime::from_timestamp(trade.match_time_unix.unwrap_or_else(|| Utc::now().timestamp()), 0)
            .unwrap_or_else(Utc::now)
            .to_rfc3339(),
        "outcome": outcome,
...
pub(crate) async fn build_observed_outcome(
...
pub(crate) fn compare_expected_to_observed(
